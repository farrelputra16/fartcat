      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 32 }}>
          ...
          <div className="reveal" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{
              ...
              background: error ? 'var(--red)' : dotOn ? 'var(--green)' : 'rgba(62,207,106,0.3)',
            ...
            {lastFetch && !error && (
              <span style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.05em' }}>
                — updated {fmtDate(lastFetch.toISOString())}
              </span>
            )}
          </div>
        </div>
        ...
        {loading && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, border: '1px solid var(--border-dim)', borderRadius: 4, overflow: 'hidden' }}>
            {[...Array(8)].map((_, i) => (
              <div key={i} style={{ padding: '20px 20px', background: 'var(--bg-raised)', minHeight: 80 }}>
                <div style={{ height: 8, background: 'var(--border)', borderRadius: 2, width: '60%', marginBottom: 8, animation: 'pulse-glow 1.5s ease-in-out infinite' }} />
        ...
        {!loading && k && (
          <>
            ...
            <div className="reveal" style={{
              ...
              {/* Total Distributed */}
              <div style={{ padding: '28px 28px', background: 'rgba(62,207,106,0.05)', borderRight: '1px solid var(--border-dim)' }}>
                ...
                <div style={{ fontSize: 36, fontWeight: 700, color: 'var(--green)', lineHeight: 1, marginBottom: 4, fontFamily: 'var(--font-mono)' }}>
                  {fmt(k.distributedTokens, k.distributedTokens < 100 ? 4 : 2)}
                </div>
                ...
                <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 10, color: 'var(--text-dim)' }}>{fmtNumber(k.payoutCount)} payouts processed</span>
                </div>
              </div>

              {/* Pending / Waiting */}
              <div style={{ padding: '28px 28px', background: 'rgba(232,160,48,0.04)' }}>
                <div style={{ fontSize: 9.5, color: 'var(--text-muted)', letterSpacing: '0.16em', textTransform: 'uppercase', marginBottom: 8 }}>
                  Pending Distribution
                </div>
                <div style={{ fontSize: 36, fontWeight: 700, color: 'var(--amber)', lineHeight: 1, marginBottom: 4, fontFamily: 'var(--font-mono)' }}>
                  {k.pendingTokens > 0.001 ? fmt(k.pendingTokens, 4) : k.pendingTokens.toExponential(2)}
                </div>
                <div style={{ fontSize: 18, color: 'var(--text-sub)', fontFamily: 'var(--font-mono)' }}>
                  {fmtUSD(k.pendingUsd)}
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, border: '1px solid var(--border-dim)', borderRadius: 4, overflow: 'hidden', marginTop: 1 }}>
              {[
                { label: 'Holders', value: fmtNumber(k.holderCount), sub: 'wallets holding $FARTCAT', c: 'var(--text-bright)' },
            ...
            <div className="reveal" style={{
              ...
              gap: 1, border: '1px solid var(--border-dim)', borderRadius: 4, overflow: 'hidden', marginTop: 1,
              transitionDelay: '400ms',
            }}>
              {[
                { label: `${k.quoteSymbol} in Tax Vault`, value: fmt(k.pendingTaxTokens, 2), sub: fmtUSD(k.pendingTaxUsd), c: 'var(--amber)' },
                { label: 'Fee Collected', value: '2.5%', sub: 'operating fee bps', c: 'var(--text-sub)' },
                { label: 'Last Payout', value: fmtDate(k.lastPayoutAt), sub: 'UTC timestamp', c: 'var(--text-sub)' },
              ].map((m, i) => (
                <div key={i} style={{
                  padding: '16px 20px',
                  background: i === 0 ? 'rgba(232,160,48,0.03)' : 'var(--bg-surface)',
        ...
        {/* Error state */}
        {!loading && error && (
          <div className="terminal" style={{ padding: '32px', textAlign: 'center' }}>
            <div style={{ fontSize: 20, color: 'var(--red)', marginBottom: 8 }}>!</div>
